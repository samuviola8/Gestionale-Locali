...
import { getTenantModules, type ModuleState } from "@/lib/modules";
...
// Cosa non esce dal locale, detto in una parola sola. Si segna l'eccezione e
// mai la regola: un elenco in cui ogni riga dice "si porta via" non si legge
// piu', e quello che serve sapere a colpo d'occhio e' cosa resta in sala.
// I canali spenti non parlano: in un locale senza consegne, "non a domicilio"
// e' vero per tutto il menu e non significa niente.
function fuoriCanale(p: MenuProduct, modules: ModuleState): string | null {
  // Le spunte contano solo dove il menu si vede da fuori: senza gli ordini dal
  // sito, "non in asporto" e' vero e non serve a niente.
  if (!modules.web_orders) return null;
  const asporto = modules.takeaway && !p.takeawayAvailable;
  const domicilio = modules.delivery && !p.deliveryAvailable;
  if (asporto && domicilio) return "solo in sala";
  if (asporto) return "non in asporto";
  if (domicilio) return "non a domicilio";
  return null;
}
...
export default async function ElencoProdotti({
  ...
  // I canali del locale decidono quali spunte hanno senso: in un pub senza
  // asporto, "si porta via" e' una domanda a cui nessuno deve rispondere.
  ...
  return (
    <div className="card divide-y" style={{ borderColor: "var(--border)" }}>
      {prodotti.map((p) => {
        // Un prodotto per volta: l'etichetta si calcola qui e non dentro al
        // JSX, dove sarebbe la stessa chiamata scritta due volte.
        const fuori = fuoriCanale(p, modules);
          ...
          >
            ...
            <div className="min-w-[180px] flex-1">
              <div className={p.available ? "" : "opacity-45"}>
                ...
                {fuori && <span className="badge badge-warn ml-2">{fuori}</span>}
            ...
            <details className="disclosure w-full">
              ...
              <div className="disclosure-body">
                <form action={updateProduct} className="grid gap-3 sm:grid-cols-2">
                  ...
                  {/* Dove si puo' ordinare. Compaiono solo se il locale quei
                      canali ce li ha: altrimenti sono due domande senza
                      risposta in fondo a ogni prodotto del menu. */}
                  {modules.web_orders &&
                      ...
                      {modules.takeaway && (
                        <label className="flex items-start gap-2.5 text-sm">
                          <input
                            type="checkbox"
                            name="asporto"
                            defaultChecked={p.takeawayAvailable}
                            className="mt-px"
                          />
                          <span>
                      ...
                      {modules.delivery && (
                        <label className="flex items-start gap-2.5 text-sm">
                          <input
                            type="checkbox"
                            name="domicilio"
                            defaultChecked={p.deliveryAvailable}
                            className="mt-px"
                          />
                          <span>
                            ...
                            <span
                              ...
                              style={{ color: "var(--muted)" }}
                            >
                              Spenta, non si può ordinare a domicilio: il fritto
                              arriva molle, il cocktail non viaggia.
                            </span>
                          </span>
                        </label>
                      )}
